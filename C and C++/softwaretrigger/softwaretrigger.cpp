...
static U32 _stdcall		ImageCallbackFunction(HANDLE hCamera, void* pFrameData, U32 dataFormat, FRAME_DESC const * pFrameDesc, void* context);
...
main(const int argc, char const * const argv[])
{
	...
	for(size_t i=0; i < serialNumbers.size(); i++) {
		...
		if (API_SUCCESS(PrepareCameraForTest(hCamera))) {
			printf("Camera %d has handle 0x%8.8X\n", serialNumbers[i], hCamera);
	...
	if (hCameras.size() == numberOfCameras) {
		const U32 numberOfCapturesToDo = 20;
...
// Note that when a camera is configured for software triggering that although the camera is in 
// streaming mode, the LED on the back of the camera is not blinking. 
//
...
{
	printf("Callback on thread 0x%8.8X for camera 0x%8.8X, frameTime = %f\n", GetCurrentThreadId(), hCamera, pFrameDesc->fFrameTime);
	return ApiSuccess;
}
...
//				printf("Software trigger sent to 0x%8.8X\n", hCameras[i]);
			}
